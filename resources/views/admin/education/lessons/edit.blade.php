...
<div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
    <div>
        <h4 class="mb-0 fw-bold text-dark"><i class="bi bi-pencil me-2 text-primary"></i>Edit Lesson</h4>
    ...
    <div class="d-flex gap-2 flex-wrap">
        <a href="{{ route('admin.courses.lessons.show', [$course, $lesson]) }}" class="btn btn-outline-info btn-sm fw-semibold">
...
<form method="POST" action="{{ route('admin.courses.lessons.update', [$course, $lesson]) }}" enctype="multipart/form-data" id="lessonForm">
    @csrf
    @method('PUT')
    {{-- Hidden field for auto-detected video duration in minutes --}}
    <input type="hidden" name="duration_minutes" id="durationMinutesInput" value="{{ old('duration_minutes', $lesson->duration_minutes ?? 0) }}">
            ...
            <div class="card border-0 shadow-sm mb-4" style="border-radius: 14px;">
                ...
                <div class="card-body p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $lesson->title) }}" placeholder="e.g. Introduction to Forex Risk Management" required maxlength="255">
                        @error('title')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-3">
                        <label class="form-label fw-semibold text-secondary">Short Overview / Description</label>
                        <textarea name="description" class="form-control @error('description') is-invalid @enderror" rows="2" maxlength="1000" placeholder="Brief 1-2 line summary of what students will learn...">{{ old('description', $lesson->description) }}</textarea>
                        @error('description')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="mb-4">
                        <label class="form-label fw-semibold text-secondary">Lesson Content (Notes / Markdown / HTML)</label>
                        <textarea name="content" class="form-control @error('content') is-invalid @enderror" rows="8" maxlength="50000" placeholder="Detailed lesson study notes, trading formulas, key takeaways...">{{ old('content', $lesson->content) }}</textarea>
                    ...
                    <div class="p-3 bg-light rounded-3 border mb-3">
                        <div class="d-flex align-items-center justify-content-between mb-3 flex-wrap gap-2">
                            ...
                            <div id="durationBadgeContainer" class="{{ ($lesson->duration_minutes || $lesson->video_url) ? '' : 'd-none' }}">
                                <span class="badge bg-success-subtle text-success border border-success-subtle px-3 py-2" id="durationBadgeText" style="font-size: 0.8rem;">
                                    <i class="bi bi-clock-history me-1"></i>Duration: <strong id="formattedDuration">{{ $lesson->duration_minutes ? $lesson->duration_minutes . ' min' : 'Auto' }}</strong>
                                </span>
                            </div>
                        </div>
                        ...
                        {{-- Current Video Banner (If Exists) --}}
                        @if($lesson->video_url)
                            ...
                            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                                <div class="d-flex align-items-center gap-2 overflow-hidden">
                                    ...
                                    <div class="overflow-hidden">
                                        <div class="fw-bold text-dark small text-truncate">Current Active Video</div>
                                        <small class="text-secondary text-truncate d-block" style="max-width: 380px;">{{ $lesson->video_url }}</small>
                                    </div>
                                </div>
                        ...
                        <ul class="nav nav-pills nav-fill mb-3" id="videoSourceTab" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active py-2 fw-semibold" id="upload-tab" data-bs-toggle="tab" data-bs-target="#upload-pane" type="button" role="tab">
                                    <i class="bi bi-cloud-arrow-up-fill me-1"></i>{{ $lesson->video_url ? 'Upload New Video' : 'Upload Video File' }}
                                </button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link py-2 fw-semibold" id="url-tab" data-bs-toggle="tab" data-bs-target="#url-pane" type="button" role="tab">
                                    <i class="bi bi-link-45deg me-1"></i>Video URL
                                </button>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content" id="videoSourceTabContent">
                            ...
                            <div class="tab-pane fade show active" id="upload-pane" role="tabpanel">
                                <div class="border-2 border-dashed rounded-3 p-4 text-center bg-white" style="border: 2px dashed #cbd5e1; cursor: pointer;" onclick="document.getElementById('videoFileInput').click()">
                                    ...
                                    <div class="fw-bold text-dark mb-1">Click to browse or drop new video file here</div>
                            ...
                            <div class="tab-pane fade" id="url-pane" role="tabpanel">
                                <div class="bg-white p-3 rounded-3 border">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="url" name="video_url" id="videoUrlInput" class="form-control @error('video_url') is-invalid @enderror" value="{{ old('video_url', $lesson->video_url) }}" placeholder="https://www.youtube.com/watch?v=... or direct MP4 link" oninput="handleVideoUrlInput(this.value)">
                                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" name="sort_order" class="form-control @error('sort_order') is-invalid @enderror" value="{{ old('sort_order', $lesson->sort_order) }}" min="0">
            ...
            <div class="card border-0 shadow-sm mb-4" style="border-radius: 14px;">
                ...
                <div class="card-body p-4">
                    <div class="form-check form-switch mb-3">
                        ...
                        <input type="checkbox" name="is_published" class="form-check-input" id="isPublished" value="1" {{ old('is_published', $lesson->is_published) ? 'checked' : '' }}>
                        <label class="form-check-label fw-semibold text-dark" for="isPublished">Published</label>
                    </div>
                    <div class="mb-3"><small class="text-secondary"><i class="bi bi-eye me-1"></i>Total Views: {{ number_format($lesson->views_count) }}</small></div>
        ...
        form.addEventListener('submit', function(e) {
            ...
            if (!hasFile && !hasUrl) {
                ...
                Swal.fire({
                    ...
                    text: 'Please upload a video file (MP4/MOV/WebM) or provide a video URL before updating the lesson.',
